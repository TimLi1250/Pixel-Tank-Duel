...
#define SHOOT_PIN     6
*/
enum { ANGLE_DEC_PIN=2, ANGLE_INC_PIN=3, POWER_DEC_PIN=4, POWER_INC_PIN=5, SHOOT_PIN=6 };
...
void setup() {
  ...
  for (int i = 0; i < NUM_BUTTONS; i++) {
    ...
    buttons[i].debouncedState  = initial;
...
void loop() {
  ...
  // debounce for every button
  for (int i = 0; i < NUM_BUTTONS; i++) {
    bool raw = digitalRead(buttons[i].pin);
    if (raw != buttons[i].lastReading) {
      buttons[i].lastChange = now;
      buttons[i].lastReading = raw;
    }
    if ((now - buttons[i].lastChange) > DEBOUNCE_MS
        && buttons[i].debouncedState != raw) {
      buttons[i].debouncedState = raw;
    }
  }

  // Unpack debounced states
  bool ad = buttons[0].debouncedState;
  bool ai = buttons[1].debouncedState;
  bool pd = buttons[2].debouncedState;
  bool pi = buttons[3].debouncedState;
  bool sh = buttons[4].debouncedState;
